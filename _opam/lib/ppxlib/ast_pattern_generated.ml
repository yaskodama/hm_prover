...
let labelled (T f0) =
  T
    (fun ctx loc x k ->
       match x with
       | Labelled x0 ->
           (ctx.matched <- (ctx.matched + 1); (let k = f0 ctx loc x0 k in k))
       | _ -> fail loc "Labelled")
let optional (T f0) =
  T
    (fun ctx loc x k ->
...
let pcl_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pcl_attributes = [] } in let k = f2 ctx loc x k in k)
       ...
       (let loc = x.pcl_loc in
        ...
        | Pcl_constr (x0, x1) ->
            (ctx.matched <- (ctx.matched + 1);
             (let k = f0 ctx x0.loc x0.txt k in let k = f1 ctx loc x1 k in k))
        | _ -> fail loc "constr"))
...
let pcf_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pcf_attributes = [] } in let k = f2 ctx loc x k in k)
...
let class_infos_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pci_attributes = [] } in let k = f2 ctx loc x k in k)
...
let pcty_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pcty_attributes = [] } in let k = f2 ctx loc x k in k)
...
let pctf_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pctf_attributes = [] } in let k = f2 ctx loc x k in k)
...
let constructor_declaration_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pcd_attributes = [] } in let k = f2 ctx loc x k in k)
...
let ptyp_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with ptyp_attributes = [] } in let k = f2 ctx loc x k in k)
let ptyp_any =
  T
    (fun ctx _loc x k ->
  ...
  T
    (fun ctx _loc x k ->
       ...
       (let loc = x.ptyp_loc in
        ...
        | Ptyp_alias (x0, x1) ->
            (ctx.matched <- (ctx.matched + 1);
             (let k = f0 ctx loc x0 k in let k = f1 ctx x1.loc x1.txt k in k))
        | _ -> fail loc "alias"))
...
let pexp_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pexp_attributes = [] } in let k = f2 ctx loc x k in k)
let pexp_ident (T f0) =
  T
    (fun ctx _loc x k ->
        ...
        | Pexp_let (x0, x1, x2) ->
            (ctx.matched <- (ctx.matched + 1);
             (let k = f0 ctx loc x0 k in
              let k = f1 ctx loc x1 k in let k = f2 ctx loc x2 k in k))
        | _ -> fail loc "let"))
let pexp_function (T f0) (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       Common.assert_no_attributes x.pexp_attributes;
       (let loc = x.pexp_loc in
    ...
    (fun ctx _loc x k ->
       ...
       (let loc = x.pexp_loc in
        ...
        | Pexp_setfield (x0, x1, x2) ->
            (ctx.matched <- (ctx.matched + 1);
             (let k = f0 ctx loc x0 k in
              let k = f1 ctx x1.loc x1.txt k in let k = f2 ctx loc x2 k in k))
        | _ -> fail loc "setfield"))
let pexp_array (T f0) =
  T
    (fun ctx _loc x k ->
...
let extension_constructor_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pext_attributes = [] } in let k = f2 ctx loc x k in k)
  ...
  T
    (fun ctx loc x k ->
       ...
       | Pfunction_body x0 ->
           (ctx.matched <- (ctx.matched + 1); (let k = f0 ctx loc x0 k in k))
       | _ -> fail loc "body")
let pfunction_cases (T f0) (T f1) (T f2) =
  T
    (fun ctx loc x k ->
       match x with
       | Pfunction_cases (x0, x1, x2) ->
           (ctx.matched <- (ctx.matched + 1);
            (let k = f0 ctx loc x0 k in
             let k = f1 ctx loc x1 k in let k = f2 ctx loc x2 k in k))
       | _ -> fail loc "cases")
let pparam_loc (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       let loc = x.pparam_loc in
       let k = f1 ctx loc loc k in let k = f2 ctx loc x k in k)
let pparam_val (T f0) (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       let loc = x.pparam_loc in
       let x = x.pparam_desc in
       match x with
       | Pparam_val (x0, x1, x2) ->
           (ctx.matched <- (ctx.matched + 1);
            (let k = f0 ctx loc x0 k in
...
let label_declaration_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pld_attributes = [] } in let k = f2 ctx loc x k in k)
let label_declaration ~name:(T name) ~mutable_:(T mutable_) ~type_:(T type_)
  =
  T
    (fun ctx loc x k ->
...
let letop ~let_:(T let_) ~ands:(T ands) ~body:(T body) =
  T
    (fun ctx loc x k ->
       ...
       let k = ands ctx loc x.ands k in let k = body ctx loc x.body k in k)
...
let module_binding_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pmb_attributes = [] } in let k = f2 ctx loc x k in k)
...
let module_declaration_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pmd_attributes = [] } in let k = f2 ctx loc x k in k)
...
let pmod_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pmod_attributes = [] } in let k = f2 ctx loc x k in k)
let pmod_ident (T f0) =
  T
    (fun ctx _loc x k ->
...
let module_substitution_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pms_attributes = [] } in let k = f2 ctx loc x k in k)
  ...
  T
    (fun ctx _loc x k ->
       ...
       (let k = name ctx (x.pms_name).loc (x.pms_name).txt k in
        let k = manifest ctx (x.pms_manifest).loc (x.pms_manifest).txt k in k))
...
let pmty_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pmty_attributes = [] } in let k = f2 ctx loc x k in k)
let pmty_ident (T f0) =
  T
    (fun ctx _loc x k ->
...
let module_type_declaration_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pmtd_attributes = [] } in let k = f2 ctx loc x k in k)
...
let pof_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pof_attributes = [] } in let k = f2 ctx loc x k in k)
let otag (T f0) (T f1) =
  T
    (fun ctx _loc x k ->
...
let ppat_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with ppat_attributes = [] } in let k = f2 ctx loc x k in k)
let ppat_any =
  T
    (fun ctx _loc x k ->
...
let prf_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with prf_attributes = [] } in let k = f2 ctx loc x k in k)
  ...
  T
    (fun ctx loc x k ->
       ...
       | Ptop_dir x0 ->
           (ctx.matched <- (ctx.matched + 1); (let k = f0 ctx loc x0 k in k))
       | _ -> fail loc "dir")
let pconstraint (T f0) =
  T
    (fun ctx loc x k ->
       match x with
       | Pconstraint x0 ->
           (ctx.matched <- (ctx.matched + 1); (let k = f0 ctx loc x0 k in k))
       | _ -> fail loc "Pconstraint")
let pcoerce (T f0) (T f1) =
  T
    (fun ctx loc x k ->
       match x with
       | Pcoerce (x0, x1) ->
...
let value_binding_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pvb_attributes = [] } in let k = f2 ctx loc x k in k)
let value_binding ~pat:(T pat) ~expr:(T expr) ~constraint_:(T constraint_) =
  T
    (fun ctx loc x k ->
       Common.assert_no_attributes x.pvb_attributes;
       (let k = pat ctx loc x.pvb_pat k in
        let k = expr ctx loc x.pvb_expr k in
        let k = constraint_ ctx loc x.pvb_constraint k in k))
let pvc_constraint (T flocally_abstract_univars) (T ftyp) =
  T
    (fun ctx loc x k ->
       match x with
       | Pvc_constraint { locally_abstract_univars; typ } ->
           (ctx.matched <- (ctx.matched + 1);
            (let k =
               flocally_abstract_univars ctx loc locally_abstract_univars k in
             let k = ftyp ctx loc typ k in k))
       | _ -> fail loc "constraint")
let pvc_coercion (T fground) (T fcoercion) =
  T
    (fun ctx loc x k ->
       match x with
       | Pvc_coercion { ground; coercion } ->
           (ctx.matched <- (ctx.matched + 1);
            (let k = fground ctx loc ground k in
             let k = fcoercion ctx loc coercion k in k))
       | _ -> fail loc "coercion")
let value_description_attributes (T f1) (T f2) =
  T
    (fun ctx _loc x k ->
       ...
       let x = { x with pval_attributes = [] } in let k = f2 ctx loc x k in k)
